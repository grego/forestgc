...
pub struct Graph {
    pub num_vertices: u8,
    pub edges: Vec<(u8, u8)>,
    pub adj: Vec<Vec<bool>>, // optional adjacency bit matrix
}
...
impl Graph {
    pub fn new(num_vertices: u8, edges: Vec<(u8, u8)>) -> Self {
        let mut adj = vec![vec![false; num_vertices as usize]; num_vertices as usize];
        for &(u, v) in &edges {
    ...
    pub fn canonical_label(&self, init_colors: Option<&[usize]>) -> (Graph, Vec<u8>) {
        let n = self.num_vertices as usize;
        ...
        if let Some(colors) = init_colors {
            ...
            return self.canonical_label(Some(&deg));
...
fn test_canonical_label_random(n_tests: usize) {
    ...
    for i in 0..n_tests {
        ...
        let (can1, _) = g.canonical_label(None);
        // println!("Canonical A: {} ", can1.to_g6());
        let (can2, _) = g2.canonical_label(None);
...
fn main() {
    ...
    test_canonical_label_file("data/graphs9_0.g6",1000);
    test_canonical_label_file("data/graphs9_1.g6",1000);
    test_canonical_label_file("data/graphs9_2.g6",1000);
